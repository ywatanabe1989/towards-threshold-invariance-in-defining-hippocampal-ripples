...
def cvt_lpath_lfp_2_lpath_rip(lpath_lfp, **kwargs):
  ...
  lpath_rip = lpath_lfp.replace(lsamp_str, '1kHz').replace('.npy', '_riptimes_sd1.pkl')
  return lpath_rip
...
def define_Xb_Tb(lfp, rip_sec, **kwargs):
  ...
  print(np.unique(outputs['Tb_levels']))
  np.unique( (outputs['Tb_levels'] * (outputs['Tb_levels'] != -1)).sum(axis=1) )
  print(outputs.keys())
  ...
  rip_sec_cp = rip_sec_cp[['start_sec', 'end_sec', 'level']]
  ...
  rips_level_in_slices = [np.array(rip_sec_cp_arr[:,2][in_slice_rip_indi[i]], dtype=np.int) for i in range(len(slices))]
  ...
  Tb_levels = pad_sequence(rips_level_in_slices, padding_value=-1)

  ########## Sort out training data ##########
  ## Filter out only level 0 label
  indi_level_k = (Tb_levels * (Tb_levels != -1)).sum(axis=1) > 0
  Xb, Tb_CenterX_W, Tb_levels = Xb[indi_level_k], Tb_CenterX_W[indi_level_k], Tb_levels[indi_level_k]

  ## Filter out Width == 0 samples
  indi_width_0 = (Tb_CenterX_W[:,:,1] == 0).any(axis=1)
  Xb, Tb_CenterX_W, Tb_levels = Xb[~indi_width_0], Tb_CenterX_W[~indi_width_0], Tb_levels[~indi_width_0]

  ## Filter out level < 5 labels and the corresponding samples
  k = 5
  indi_less_than_level_k = (Tb_levels * (Tb_levels < k) * (Tb_levels != -1)).any(axis=1)
  Xb, Tb_CenterX_W, Tb_levels = Xb[~indi_less_than_level_k], Tb_CenterX_W[~indi_less_than_level_k], Tb_levels[~indi_less_than_level_k]

  ## Filter out 200 ms < Ripple width samples
  w_100ms = 100/Xb.shape[-1]
  mask_level_5 = (Tb_levels == 5)
  ...
  Tb_levels = Tb_levels[indi_level_5_less_than_200ms]

  # Find samples which start and end at level 0
  indi_zero_start = (Tb_levels[:, 0] == 0)
  pad_len = (Tb_levels == -1).sum(axis=-1)
  last_levels = np.array([Tb_levels[i, Tb_levels.shape[-1] - (pad_len[i]+1)] for i in range(len(pad_len))])
  indi_zero_end = (last_levels == 0)
  indi_level_k_inserted = indi_zero_start * indi_zero_end
  Xb = Xb[indi_level_k_inserted]
  Tb_CenterX_W =  Tb_CenterX_W[indi_level_k_inserted]
  Tb_levels = Tb_levels[indi_level_k_inserted]

  # Mask out level 0,1,2,3,4 targets
  not_level_k_mask = (Tb_levels != k)
  Tb_levels[not_level_k_mask], Tb_CenterX_W[not_level_k_mask] = -1, np.nan

  # Sorting
  indi_sort = np.argsort(Tb_levels)[...,::-1]
  Tb_levels = np.take_along_axis(Tb_levels, indi_sort, axis=-1)
  Tb_CenterX_W = np.take_along_axis(Tb_CenterX_W, indi_sort[..., np.newaxis], axis=1)

  ## Check
  # print(np.unique(Tb_levels))

  outputs = {'Xb':Xb, 'Tb_CenterX_W':Tb_CenterX_W, 'Tb_levels':Tb_levels}
...
def multi_define_Xb_Tb_wrapper(lfps, rips_sec, **kwargs):
  ...
  def gathers_mp_outputs_dict_list(mp_outputs_dict_list, **kwargs): # Xb_Tb_keys=None
    ...
    dtype = gathered_dict['Tb_levels'][0].dtype
    for i in range(len(gathered_dict['Tb_levels'])):
      s1, s2 = gathered_dict['Tb_levels'][i].shape
      if max_n_bounding_ranges_in_an_input - s2 > 0:
        minus_ones_arr = (np.ones([s1, max_n_bounding_ranges_in_an_input - s2]) * -1).astype(dtype)
        gathered_dict['Tb_levels'][i] = np.concatenate([gathered_dict['Tb_levels'][i], minus_ones_arr], axis=1)
    gathered_dict['Tb_levels'] = np.concatenate(gathered_dict['Tb_levels'])
...
def pack_Xb_Tb_to_dataloader(Xb_Tbs_dict, **kwargs):
  ...
  def collate_fn(batch):
      ...
      Tb_levels = torch.stack([item[2] for item in batch], dim=0)
      # targets_mask = (Tb_levels != -1) & (Tb_levels != 0)
      targets_mask = (Tb_levels != -1)
      # targets_mask = Tb_levels != -1
      ids = torch.arange(len(targets_mask)).unsqueeze(-1).expand(-1, Tb_CenterX_W.shape[1])
      dtype = Xb.dtype

      targets_ids = ids[targets_mask].unsqueeze(-1)
      targets_levels = Tb_levels[targets_mask].unsqueeze(-1)
...
if __name__ == '__main__':
  ...
  batch = next(iter(dl))
  Xb, targets = batch
  _check_samples(Xb, targets, max_plot=10)
  '''
