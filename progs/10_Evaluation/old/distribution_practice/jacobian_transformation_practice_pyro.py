...
x0 = np.linspace(0, 50, point_num, endpoint=False)
y1 = f1(x0)
plt.plot(x0, y1, label='y = f1(x)')
plt.legend()
plt.show()
...
sigma = 1
point_num = 2e9
x_px = np.linspace(0, point_num, point_num, endpoint=False)
...
class my_norm_gen(stats.rv_continuous):
  def _pdf(self, x):
    return normal_pdf(x, 0., 1.)

class pdf_y_gen(stats.rv_continuous):
  def _pdf(self, x):
    # y = f1(x)
    return pdf_y(x, 0., 3.)

class pdf_z_gen(stats.rv_continuous):
  def _pdf(self, x):
    z = f2(f1(x))
    return pdf_z(z, 0., 1.)

deterministic = deterministic_gen(name="deteministic")
deterministic.cdf(np.arange(-3, 3, .5))
deterministic.pdf(np.arange(-3, 3, .5))

mynorm = my_norm_gen()
mynorm.pdf(10)


mypdf_y = pdf_y_gen(name='pdf_y')
mypdf_y.pdf(10)


mypdf_z = pdf_z_gen(name='pdf_z')
mypdf_z.pdf(10)

from scipy import integrate
integrate.quad(deterministic.pdf, -np.inf, np.inf)
integrate.quad(deterministic.pdf, -1e-3, 1e-3)
