...
class Recorder():
    ...
    def add_SD(Tb_SD):
        self.Tb_SD.append(Tb_SD.item)
    def add_prob(pred_isRipple_prob):
        self.pred_isRipple_prob.append(pred_isRipple_prob.item)


def test(n_batches=10):
  global cls_container, recorder
  recorder = Recorder()
  ...
  with torch.no_grad():
    for i_batch, batch in enumerate(dl_tes):
      ...
      if m['use_loss'] == 'xentropy':
        ...
        Tb_SD = Xb_Tbs_dict['Tb_SD'].to(torch.long)
        ...
        recorder.add_SD(pred_isRipple_probs)
