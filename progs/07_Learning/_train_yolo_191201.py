...
def calc_anchors(dl_fulf, n_epochs=1, bs=256, n_clusters=9):
    ...
    for epoch in range(n_epochs):
        ...
        for i_batch, batch in enumerate(dl):
            Xb, targets = batch
            kmeans = kmeans.partial_fit(targets[:, 3].unsqueeze(-1))
...
def train(model, dl_fulf_tra, d, p, m):
  ...
  for i_batch, batch in enumerate(dl_tra):
    ...
    loss, output, metrics = model(Xb, targets=targets)
...
def plot_prediction(dl_tra):
  ...
  targets = targets[:10]
  outputs = model(Xb)
  outputs = nms(outputs, conf_thres=0.5, nms_thres=0.)
  print(targets)
  pprint(outputs)
