...
from macro_double_soft_f1 import macro_double_soft_f1
from losses import FocalLoss
# from focal_loss import FocalLoss
# from focal_loss import FocalLossWithOutOneHot
...
men3; CUDA_VISIBLE_DEVICES=0,1,2,3; python 07_Learning/train_thesis_200109.py -nmt 01;\
men3; CUDA_VISIBLE_DEVICES=0,1,2,3; python 07_Learning/train_thesis_200109.py -nmt 02;\
men3; CUDA_VISIBLE_DEVICES=0,1,2,3; python 07_Learning/train_thesis_200109.py -nmt 03;\
men3; CUDA_VISIBLE_DEVICES=0,1,2,3; python 07_Learning/train_thesis_200109.py -nmt 04;\
men3; CUDA_VISIBLE_DEVICES=0,1,2,3; python 07_Learning/train_thesis_200109.py -nmt 05;\
'''

ap = argparse.ArgumentParser(formatter_class=argparse.ArgumentDefaultsHelpFormatter)
ap.add_argument("-nmt", "--n_mouse_tes", default="01", choices=["01", "02", "03", "04", "05"], help = " ") # nargs='+',
ap.add_argument("-l", "--use_loss", default='f1', choices=['bce', 'xentropy', 'focal', 'kldiv', 'f1'], help = " ")
...
ap.add_argument("-nl", "--n_load_all", default=184, choices=[12, 184], type=int, help = " ")
...
ap.add_argument("-bf", "--batch_factor", default=7, type=int, help=" ")
...
ap.add_argument("-msl", "--max_seq_len", default=400, type=int, help=" ")
...
# args.n_load_all = 12 # fixme
# ########################

if args.use_loss == 'f1':
    args.apply_n_labels_balancing = False
else:
...
def train():
  ...
  for i_batch, batch in enumerate(dl_tra):
    ...
    if m['use_loss'] == 'focal':
      ...
      # loss_isRipple = focal_criterion(pred_isRipple_logits, Tb_isRipple.unsqueeze(-1)) # fixme: Tb_isRipple
      pred_isRipple_probs = sigmoid(pred_isRipple_logits)
      loss_isRipple = focal_criterion(pred_isRipple_probs.squeeze(), Tb_isRipple.to(torch.float)) # fixme: Tb_isRipple
      pred_isRipple_cls = (pred_isRipple_probs > .5)
      cls_rec.add_target(Tb_isRipple.detach().cpu().numpy())
      cls_rec.add_output(pred_isRipple_cls.detach().cpu().numpy())
    ...
    if m['use_loss'] == 'f1':
      Tb_isRipple = Xb_Tbs_dict['Tb_label'].to(torch.long)
      pred_isRipple_probs = sigmoid(pred_isRipple_logits)
      loss_isRipple = f1_criterion(Tb_isRipple.float(), pred_isRipple_probs.float(), reduction='mean') # fixme: Tb_isRipple
      pred_isRipple_cls = (pred_isRipple_probs > 0.5).long()
      cls_rec.add_target(Tb_isRipple.detach().cpu().numpy())
      cls_rec.add_output(pred_isRipple_cls.detach().cpu().numpy())
...
def define_parameters():
  ...
  if (args.use_loss == 'bce') or (args.use_loss == 'focal') or (args.use_loss == 'f1'):
    m['n_out'] = 1
...
def init_dataloaders():
  ...
  kwargs_dl_tra = {'ripples_binary_classification':True,
                   ...
                   'label_name':'label_cleaned_from_gmm_wo_mouse{}'.format(args.n_mouse_tes),
                   }
  #                  'label_name':None,

  if kwargs_dl_tra['label_name'] is None:
...
def init_ripple_detector_NN():
  ...
  if args.use_loss == 'focal':
    ...
    sigmoid = torch.nn.Sigmoid()
    focal_criterion = FocalLoss(alpha=2., beta=6.) # alpha=2, beta=4
    # focal_criterion = FocalLoss(gamma=2., alpha=0.25) # gamma=2. , alpha=.25
    # focal_criterion = FocalLoss(2, gamma=2.) # gamma=2. , alpha=.25
    # focal_criterion = FocalLossWithOutOneHot(gamma=2.)

  if args.use_loss == 'f1':
    global f1_criterion
    sigmoid = torch.nn.Sigmoid()
    f1_criterion = macro_double_soft_f1
