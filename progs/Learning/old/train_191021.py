...
def _save_tra(i_batch, cum_dict):
  ...
  try:
    d['cm_isRipple_tra'].append(cum_dict['cm_isRipple_tra'])
    d['cr_isRipple_tra'].append(cum_dict['cr_isRipple_tra'])
  except:
    pass
...
class class_recorder():
  ...
  def conf_mat(self, labels=None):
    self.concat()
    return mf.calc_confusion_matrix(self.targets, self.outputs, labels=labels)

  def cls_rep(self, labels=None):
    self.concat()
    return mf.report_classification_results(self.targets, self.outputs, labels=labels)
...
def train():
  ...
  keys_cum = ['cum_loss_isRipple_tra', 'cum_loss_tot_tra', 'correct_isRipple_tra', 'n_counter_tra',
              'cm_isRipple_tra', 'cr_isRipple_tra']
  cum_dict = mf.init_dict(keys_cum)

  dl_tra = dl_fulf_tra.fulfill()

  cls_rec = class_recorder()
  ...
  for i_batch, batch in tqdm(enumerate(dl_tra)):
    ...
    # batch to dict
    Xb_Tbs_dict = mf.init_dict(keys=dl_fulf_tra.kwargs['keys_to_pack'], values=batch)

    # to GPU
    for k in dl_fulf_tra.kwargs['keys_to_pack']:
    ...
    if m['use_loss'] == 'bce':
      ...
      # correct_isRipple = (pred_isRipple_cls == Tb_isRipple.to(torch.bool))
    ...
    if m['use_loss'] == 'focal':
      ...
      cls_rec.add_target(Tb_isRipple.detach().cpu().numpy())
      cls_rec.add_output(pred_isRipple_cls.detach().cpu().numpy())
      # correct_isRipple = (pred_isRipple_cls.squeeze() == Tb_isRipple.to(torch.bool))
      # cum_dict['correct_isRipple_tra'] += correct_isRipple.sum().detach().cpu().numpy()

    # try:
    #   if i_batch == 0: # fixme, when all samples are only one classes in both GT labels and pred labels, ValueError occurs.
    #     cum_dict['cm_isRipple_tra'] = mf.calc_confusion_matrix(Tb_isRipple.cpu(), pred_isRipple_cls.cpu(), \
    #                                                            labels=['noRipple', 'Ripple'])
    #     cum_dict['cr_isRipple_tra'] = mf.report_classification_results(Tb_isRipple.cpu(), pred_isRipple_cls.cpu())
    #   else:
    #     cum_dict['cm_isRipple_tra'] += mf.calc_confusion_matrix(Tb_isRipple.cpu(), pred_isRipple_cls.cpu(), \
    #                                                            labels=['noRipple', 'Ripple'])
    #     cum_dict['cr_isRipple_tra'] += mf.report_classification_results(Tb_isRipple.cpu(), pred_isRipple_cls.cpu()) # fixme
    # except:
    #   pass
    ...
    if i_batch % p['print_log_batch_interval'] == 0:
      ...
      cum_dict['conf_mat'], cum_dict['cls_report'] = cls_rec.report(labels_cm=['noRipple', 'Ripple'], labels_cr=[0, 1])
...
def init_dataloaders():
  ...
  # kwargs_tes.update({'use_shuffle':True, 'use_perturb':False, 'bs':p['bs_tes']}) # use_shuffle:False
...
def init_ripple_detector_NN():
  ...
  if m['use_scheduler'] == 'step':
    ...
    last_relat_lr = 0.1
    # gamma = np.exp(-np.log(10) / step_all)
...
def load_parameters(datadir):
  ...
  # kwargs_tes = dl_fulf_tra.kwargs.copy()
...
#   try:
#     d['cm_isRipple_tes'].append(cum_dict['cm_isRipple_tes'])
#     d['cr_isRipple_tes'].append(cum_dict['cr_isRipple_tes'])
#   except:
#     pass
...
#           cum_dict['cm_isRipple_tes'] = mf.calc_confusion_matrix(Tb_isRipple.cpu(), pred_isRipple_cls.cpu(), \
#                                                                  labels=['noRipple', 'Ripple'])
#           cum_dict['cr_isRipple_tes'] = mf.report_classification_results(Tb_isRipple.cpu(), pred_isRipple_cls.cpu())
#         else:
#           cum_dict['cm_isRipple_tes'] += mf.calc_confusion_matrix(Tb_isRipple.cpu(), pred_isRipple_cls.cpu(), \
#                                                                  labels=['noRipple', 'Ripple'])
#           cum_dict['cr_isRipple_tes'] += mf.report_classification_results(Tb_isRipple.cpu(), pred_isRipple_cls.cpu()) # fixme
#       except:
#         pass
