...
class Model(nn.Module):
    def __init__(self, input_size=1,
        ...
        if use_cnn:
          self.cnn = ResNet1D(input_size, cnn_n_filter_1st, use_activation)
        ...
        ########## Adaptor ##########
        # Shared FC
        if output_size <= 2: # bce, xentropy
          self.fc_shared = nn.Sequential(nn.Linear(last_shared_features_size, int(last_shared_features_size/2)),
                                         activation,
                                         nn.Dropout(p=dropout_fc)
                                        )
        if 2 < output_size: # kldiv
          self.fc_shared = nn.Sequential(nn.Linear(last_shared_features_size, last_shared_features_size),
                                         activation,
                                         nn.Dropout(p=dropout_fc)
                                        )
                     # MLP(last_shared_features_size, [int(last_shared_features_size/2)),], [activation,], [nn.Dropout(p=dropout_fc),])
        ...
        #                            )
        if output_size <= 2: # bce, xentropy
          self.fc_isRipple = nn.Sequential(nn.Linear(int(last_shared_features_size/2), int(last_shared_features_size/4)),
                                           activation,
                                           nn.Dropout(p=dropout_fc),
                                           nn.Linear(int(last_shared_features_size/4), output_size),
                                          )
        if 2 < output_size: # kldiv
          size_d = last_shared_features_size - output_size # diff
          self.fc_isRipple = nn.Sequential(nn.Linear(last_shared_features_size, last_shared_features_size-int(size_d/4)),
                                           activation,
                                           nn.Dropout(p=dropout_fc),
                                           nn.Linear(last_shared_features_size-int(size_d/4), last_shared_features_size-int(size_d/2)),
                                           activation,
                                           nn.Dropout(p=dropout_fc),
                                           nn.Linear(last_shared_features_size-int(size_d/2), output_size)
                                          )
          if use_loss == 'focal':
            # Initialize the last layer's bias to the prob of the negative samples will be large.
            # https://leimao.github.io/blog/Focal-Loss-Explained/
            pi = 0.01
            self.fc_isRipple[-1].bias.data.fill_(-torch.log((1-pi)/pi))
    ...
    def forward(self, inp): # inp: [batch, seq_len, n_features]
        ...
        # initialize the bias term of the last layer to some non-zero value such that the ptpt of positive samples is small and the ptpt of negative samples is large.
