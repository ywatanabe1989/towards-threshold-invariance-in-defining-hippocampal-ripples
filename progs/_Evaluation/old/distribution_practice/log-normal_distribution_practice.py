...
def get_rv_z(mu_x, sigma_x):
  mu_y = log_lat_std * mu_x + log_lat_mean
  sigma_y = log_lat_std * sigma_x
  rv_z = scipy.stats.lognorm(s=sigma_y, scale=np.exp(mu_y))
  return rv_z

def get_mode_z(mu_x, sigma_x):
  mu_y = log_lat_std * mu_x + log_lat_mean
  sigma_y = log_lat_std * sigma_x
  mode_z = np.exp(mu_y - sigma_y**2)
  return mode_z

rv_z = get_rv_z(mu_x, sigma_x)
mode_z = get_mode_z(mu_x, sigma_x)

# lwr: 0.025 percentile, med: 0.500 percentile, upr: 97.5 percentile
lwr, med, upr = rv_z.ppf(np.array([0.025, 0.500, 0.975]))
ci_95 = [lwr, med, upr]

# x = np.linspace(int(rv_z.ppf(0.001)), int(rv_z.ppf(0.999)), 10000000)
# x = np.linspace(0, int(rv_z.ppf(0.999)), 10000000)
...
# z = np.exp(y)
# plt.hist(z, bins=np.linspace(0, 100000, 100), density=True)
# plt.show()
...
# mean_z = get_mean(mean_y, sigma_y) # np.exp(mean_y + (sigma_y**2)/2)
# median_z = get_median(mean_y, sigma_y) # np.exp(mean_y)
# mode_z = get_mode(mean_y, sigma_y) # np.exp(mean_y - sigma_y**2)
# var_z = get_var(mean_y, sigma_y) # (np.exp(sigma_y**2)-1) * np.exp(2*mean_y + (sigma_y**2))
# ci_95 = get_quantile(mean_y, sigma_y, np.array([0.025, 0.500, 0.975]))
...
# ## Using Scipy
# rv_z = scipy.stats.lognorm(s=sigma_y, scale=np.exp(mean_y))
# # rv_z.mean()
# z_sampled = rv_z.rvs(size=N)
# plt.hist(z_sampled)
# plt.show()
# axis = np.linspace(0, 1e5, 1000000)
# plt.plot(axis, rv_z.pdf(axis))
# plt.show() # mode: 0.00012
