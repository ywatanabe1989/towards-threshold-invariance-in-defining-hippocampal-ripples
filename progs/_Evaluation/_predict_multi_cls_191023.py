...
def load_params(ts_load, plot=False):
  global timer, d, p, m, weight_path, datadir
  timer = mf.time_tracker()
  max_f1_macro_avg_dir, dirpaths = glob_the_best_model_dir(ts_load, plot=plot)
  d_path_last = dirpaths[-1]

  datadir = d_path_last # max_f1_macro_avg_dir
  ...
  with torch.no_grad():
    for i_batch, batch in enumerate(dl_tes):
      ...
      if m['use_loss'] == 'xentropy':
        ...
        cls_rec.add_target(Tb_isRipple.detach().cpu().numpy())
        cls_rec.add_output_cls(pred_isRipple_cls.detach().cpu().numpy())
        cls_rec.add_output_prob(pred_isRipple_probs[:, 1].detach().cpu().numpy())
...
if __name__ == '__main__':
  ...
  ap.add_argument("-nmt", "--n_mouse_tes", default=5, choices=[1,2,3,4,5], type=int, help = "") #  '191022/230633'
  ...
  '''
  cls_rec.concat()
  df = pd.DataFrame({'GT Ripple Level':cls_rec.targets, 'Predicted Ripple Prob.':cls_rec.outputs_prob})
  sns.boxplot(x='GT Ripple Level', y='Predicted Ripple Prob.', data=df)
  # sns.despine(offset=10, trim=True)

  '''

  conf_mat_tes, cls_report_tes = cls_rec.report(labels_cm=['level 0', 'level 1', 'level 2', 'level 3', 'level 4', 'level 5'],
                                                labels_cr=[0, 1, 2, 3, 4, 5]) # fixme, labels
  pprint(conf_mat_tes)
