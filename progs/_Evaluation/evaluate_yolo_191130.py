...
def check_with_eyes(ii, lfp, ripples, pred_ripples,
    ...
    _pred_ripples = pred_ripples[['start_sec', 'end_sec', 'duration_ms', 'score']]\
    ...
    for _pred_ripple in _pred_ripples.itertuples():
        ax.axvspan(_pred_ripple.start_sec, _pred_ripple.end_sec, alpha=0.3*_pred_ripple.score,
                   color='black', label='Pred. Ripples', zorder=1000) # Pred. ripple
        # ax.text((_pred_ripple.start_sec+_pred_ripple.end_sec/2)-_pred_ripple.start_sec, ymin+100, str(_pred_ripple.score))
...
fpath_pred_ripples = fpath_lfp.replace('_fp16.npy', '_ripple_pred_by_yolo.pkl')
...
for ii in range(N_plot):
    ...
    check_with_eyes(ii, lfp, ripples, pred_ripples,
                    score_thres=score_thres, label_3d_thres=-1, start_sec=start_sec, end_sec=end_sec, savefig=True)
