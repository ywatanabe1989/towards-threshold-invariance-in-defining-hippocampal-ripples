...
def mean_filtering_k(waveforms, k=1, s=1, p='none'):
    """ Conduct size k kernel's mean filtering with padding"""
    if p == 'none':
        p = 0
    if p == 'same':
        p = int(k / 2) + 1
...
class BasicBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        residual = mean_filtering_k(residual, k=7, p='none')
        residual = mean_filtering_k(residual, k=5, p='none')
        residual = mean_filtering_k(residual, k=3, p='none')
...
class CenterNet1D(nn.Module):
    def __init__(self, ):
        ...
        self.ripple_bandpass = BandPass(lo_hz=150, hi_hz=250, kernel_size=101, padding='none')
        ...
        self.block7 = BasicBlock(128, 128)
        self.block8 = BasicBlock(128, 128)
        self.block9 = BasicBlock(128, 128)
    ...
    def forward(self, x):
        ...
        # pred_width_logits = (x[:, int(x.shape[1]/2):, :].mean(dim=1))
        pred_hm_logits = x[:, :100, :].mean(dim=1) # fixme
        pred_width_logits = (x[:, 100:, :].mean(dim=1))
        # pred_offset_logits = (x[:, 120:, :].mean(dim=1))

        pred_hm_prob = pred_hm_logits.sigmoid()
        pred_width = abs(pred_width_logits)
        # pred_offset = abs(pred_offset_logits)

        return pred_hm_prob.to(dtype), pred_width.to(dtype), None # pred_offset.to(dtype)


class CenterNet1D_wo_bp(nn.Module):
    def __init__(self, ):
        super(CenterNet1D_wo_bp, self).__init__()
        ...
        self.block8 = BasicBlock(128, 128)
        self.block9 = BasicBlock(128, 128)
        self.block10 = BasicBlock(128, 128)
        self.block11 = BasicBlock(128, 128)
        self.block12 = BasicBlock(128, 128)
        self.block13 = BasicBlock(128, 128)
        self.block14 = BasicBlock(128, 128)
        self.block15 = BasicBlock(128, 128)
        self.block16 = BasicBlock(128, 128)
        self.block17 = BasicBlock(128, 128)
