...
def cvt_lpath_lfp_2_lpath_rip(lpath_lfp, **kwargs):
  ...
  lpath_rip = lpath_lfp.replace(lsamp_str, '1kHz').replace('.npy', '_ripple_candi_150-250Hz_with_prop_label3d_191201_filled.pkl')
  return lpath_rip
...
def define_Xb_Tb(lfp, rip_sec, **kwargs):
  ...
  -10: padding,
   -2: Back ground,
    ...
    1: "True" Ripples
    2: High-EMG False Ripple candi.,
  '''

  ########## Select training data ##########
  ## Select the slices including only "True" Ripples and High-EMG False Ripple candidates but not defined ones.
  include_not_defined_ripples = (Tb_cls == 0).any(axis=1)
  include_true_ripples = (Tb_cls == 1).any(axis=1)
  include_false_ripples = (Tb_cls == 2).any(axis=1)
  # indi =  include_true_ripple * (~(include_false_ripple + include_not_defined_ripple))
  indi = (include_true_ripples + include_false_ripples) & (~include_not_defined_ripples)
  ...
  indi_bg_start = (Tb_cls[:, 0] == -2) # fixme, -2 indicates the Back Ground
  ...
  indi_bg_end = (last_cls == -2) # fixme, -2 indicates the Back Ground
  indi = indi_bg_start & indi_bg_end
  Xb, Tb_CenterX_W, Tb_cls = Xb[indi], Tb_CenterX_W[indi], Tb_cls[indi]

  # Mask out samples which won't be used
  not_used_mask = (Tb_cls < 1)
  Tb_cls[not_used_mask], Tb_CenterX_W[not_used_mask] = -1, np.nan
