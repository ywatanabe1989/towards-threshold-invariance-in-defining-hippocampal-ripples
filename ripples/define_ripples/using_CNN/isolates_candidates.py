...
rips["LFP"] = [
    lfp[int(rip["start_pts"]) : int(rip["end_pts"])] for i_rip, rip in rips.iterrows()
]
rips["Ripple_Band_Magnitude"] = [
    rip_magni[int(rip["start_pts"]) : int(rip["end_pts"])]
    for i_rip, rip in rips.iterrows()
]
rips["ripple_peak_posi_pts"] = [
    int(rip["start_pts"]) + rip["Ripple_Band_Magnitude"].argmax()
    for i_rip, rip in rips.iterrows()
]

################################################################################
## Isolates each ripple candidate in wide band
################################################################################
# Isolates each ripple candidate
