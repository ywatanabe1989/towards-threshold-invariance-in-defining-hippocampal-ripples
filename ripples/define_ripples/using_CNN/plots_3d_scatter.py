...
## Plots
spath_mp4 = (
    utils.general.mk_spath(
        "{ds}_mouse_{n}.mp4".format(ds=dataset_str, n=args.n_mouse), makedirs=True
    )
    if args.save == "mp4"
    else None
)
...
spath_png = (
    utils.general.mk_spath(
        "{ds}_mouse_{n}.png".format(ds=dataset_str, n=args.n_mouse), makedirs=True
    )
    if args.save == "png"
    else None
)
