...
LPATH_HIPPO_LFP_NPY_LIST_MICE = utils.general.search_str_list(LPATH_HIPPO_LFP_NPY_LIST, args.n_mouse)[1]

lpath_lfp = LPATH_HIPPO_LFP_NPY_LIST_MICE[0]
lpath_rip_sec = utils.path_converters.LFP_to_ripples(lpath_lfp,
...
def plot_traces(start_sec=6516, dur_sec=3):
    ...
    _rip_magni_plt = rip_magni[start_pts:end_pts].squeeze()
    ...
    filted_plt, rip_magni_plt, _ = utils.dsp.define_ripple_candidates(x_sec,
                                                          lfp_plt,
                                                          samp_rate,
        ...
        for ripple in rip_sec_plt.itertuples():
            ...
            if first:
                ...
                ax[i].axvspan(ripple.start_sec, ripple.end_sec, alpha=0.1,
                              color='red', zorder=1000, label=label)
                first = False
            else:
                ax[i].axvspan(ripple.start_sec, ripple.end_sec, alpha=0.1,
...
s = np.random.randint(0, 500000)
plot_traces(start_sec=s, dur_sec=5)
...
# d = 3*1000
# lfp_plt = lfp[s:s+d]
# rip_magni_plt = rip_magni[s:s+d]
# fig, ax = plt.subplots(2, 1, sharex=True)
# ax[0].plot(lfp_plt, linewidth=linewidth,
#            label='raw LFP')
# ax[1].plot(rip_magni_plt, linewidth=linewidth,
#            label='ripple band normalized magnitude')
# fig.show()
