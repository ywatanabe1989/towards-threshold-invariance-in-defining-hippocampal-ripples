...
lfps = np.vstack([skimage.util.view_as_windows(lfp[random_start:],
                                               WINDOW_SIZE_PTS,
                                               WINDOW_SIZE_PTS)
                  for lfp, random_start in zip(lfps, random_starts_pts)]
                 )
meps = np.vstack([skimage.util.view_as_windows(mep[random_start:],
                                               WINDOW_SIZE_PTS,
                                               WINDOW_SIZE_PTS)
...
fft_freqs = np.fft.fftfreq(lfps.shape[-1], d=1./SAMP_RATE)
mask = (fft_freqs >= 0)
