...
This reopsitory was forked from https://github.com/eriklindernoren/PyTorch-YOLOv3. This repository mainly aims the following three parts:
...
[△] To check the effectiveness of 2D pretrained weights for 1D model (Transfer Learning / Fine Tuning)  
    The implementation was done. However, the efficacy is dubious at this point. This may be caused by the unsuitability of the 1-dimensionalized-COCO dataset or by the corruption of the initialized weights at the very beginning of iterations with relatively higher learning rate. 
...
```
(client) $ ssh -L 9009:localhost:6006 user@server.host.name
(server) $ tensorboard --logdir='logs' --port=6006
```
The port number 6006 is default since it was like "goog"le.
So, you don't have to designate 6006 explicitly.   
Also, you might want to configure above setting on client's ~/.ssh/config file like this
```
Host <NAME>
