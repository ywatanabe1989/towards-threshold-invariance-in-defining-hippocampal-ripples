...
## Fix seeds
utils.general.fix_seeds(os=None, random=None, np=np, torch=torch, tf=None, seed=42)
...
class CleanLabelResNet1D(BaseEstimator):
    ...
    def fit(self, X, T, sample_weight=None):
        ...
        ds_tra = torch.utils.data.TensorDataset(
            torch.FloatTensor(X), torch.LongTensor(T)
        )
...
if __name__ == "__main__":
    ...
    ## Fix seeds
    utils.general.fix_seeds(random=random, np=np)
