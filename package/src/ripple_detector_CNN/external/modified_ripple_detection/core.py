...
def _mk_bandpass_filter(samp_rate, lo_hz=150, hi_hz=250, order=100):
    ...
    nyq = 0.5 * samp_rate
    TRANSITION_BAND = 25
    bands = [0, lo_hz - TRANSITION_BAND, lo_hz, hi_hz, hi_hz + TRANSITION_BAND, nyq]
...
def _get_series_start_end_times(series):
    ...
    start_times = series.index[is_start_time].to_numpy()  # get_values()

    is_end_time = series & (~series.shift(-1).fillna(False))
    end_times = series.index[is_end_time].to_numpy()  # .get_values()
...
def threshold_by_zscore(data, time, minimum_duration=0.015, zscore_threshold=2):
