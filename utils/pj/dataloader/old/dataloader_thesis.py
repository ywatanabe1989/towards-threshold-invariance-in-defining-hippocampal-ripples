...
def cvt_lpath_lfp_2_lpath_rip(lpath_lfp, **kwargs):
    ...
    lpath_rip = lpath_lfp.replace(lsamp_str, "1kHz").replace(
        "_fp16.npy",
        "_ripple_candi_150-250Hz_with_prop_label_cleaned_from_gmm_merged_filled.pkl",
    )
    return lpath_rip
...
def load_lfp_rip_sec(lpath_lfp, **kwargs):
    ...
    lfp = (
        np.load(lpath_lfp).squeeze().astype(dtype)
...
def define_Xb_Tb(lfp, rip_sec, **kwargs):
    ...
    if label_name is None:
        are_the_1st_rips_true_ripple = ~np.isnan(
            rip_sec_cp.iloc[the_1st_rips_indi]["label_cleaned_from_gmm"]
        )
        are_the_2nd_rips_true_ripple = ~np.isnan(
            rip_sec_cp.iloc[the_2nd_rips_indi]["label_cleaned_from_gmm"]
        )
        are_the_3rd_rips_true_ripple = ~np.isnan(
            rip_sec_cp.iloc[the_3rd_rips_indi]["label_cleaned_from_gmm"]
        )
...
def multi_define_Xb_Tb_wrapper(lfps, rips_sec, **kwargs):
  ...
  """
    ...
    arg_list = [
        ((lfps[i], rips_sec[i]), kwargs) for i in range(len(lfps))
    ]  # args, kwargs
...
def pack_Xb_Tb_to_dataloader(Xb_Tbs_dict, **kwargs):
    ...
    dataset = utils.TensorDataset(
        *(torch.tensor(Xb_Tbs_dict[k]) for k in kwargs["keys_to_pack"])
    )
...
class dataloader_fulfiller:
    ...
    def fulfill(
        ...
        Xb_Tbs_dict = multi_define_Xb_Tb_wrapper(
            self.lfps, self.rips_sec, **self.kwargs
        )
