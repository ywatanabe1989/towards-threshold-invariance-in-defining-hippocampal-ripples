...
def pack_Xb_Tb_to_dataloader(Xb_Tbs_dict, **kwargs):
  ...
  Xb, targets, targets_label_conf = batch
  ...
  def collate_fn(batch):
      ...
      # To fp32
      Xb, targets, targets_label_conf = Xb.transpose(1,2).float(), targets.float(), targets_label_conf.float()

      return Xb, targets, targets_label_conf
...
class DataloaderFulfiller():
  ...
  batch = next(iter(dl))
  Xb, targets, label_conf = batch
  _check_samples(Xb, targets, label_conf=label_conf, max_plot=10)
  '''
