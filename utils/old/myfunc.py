...
def mk_MNIST_dataloader(
    mldata_dir="../../data/mldata", train=True, bs=64, nw=10, pm=True, collate_fn=None
):
    import torchvision
    ...
    if collate_fn:
        ...
        dataloader = utils.DataLoader(
            dataset, batch_size=bs, shuffle=train, num_workers=nw, pin_memory=True
        )
    return dataloader
...
class MyBayes:
    ...
    def gpr(self):
        ...
        model = gpflow.gpr.GPR(
            self.X, self.Y, gpflow.kernels.Matern52(2, ARD=True)
        )  # ???
        ...
        with optimizer.silent():
            self.r = optimizer.optimize(
                self.fx, n_iter=self.n_iter
            )  # minimize fx output
        print(self.r)
...
def batch2inputs_tra(
    batch, max_seq_len, samp_rate
):  # after getting batch from dataloader
    Xb = batch[0]
    Tb = batch[1]
    ...
    for i in range(len(Xb)):
        ...
        while not set_flag:
            try:
                slice_start = random.randint(
                    0, Xi_length - max_seq_len
                )  # min: 0, max: Xi_length - max_seq_len
                ...
                next_ripple_ind = after_slice_end_mask.nonzero()[
                    0
                ...
                from_slice_end_to_the_next_ripple_start = (
                    Ti[0][next_ripple_ind] - slice_end_sec
                )
...
def padding_from_numpylist(numpylist):
    ...
    return torch.nn.utils.rnn.pad_sequence(
        numpylist, batch_first=True, padding_value=-float("Inf")
    )
...
def balance_loss(loss, Tb, counts_arr=None, n_classes_int=None):
    ...
    probs_arr = 1.0 * counts_arr / counts_arr.sum()
...
def wavelet_scattering_1D(x, J=8, Q=12):
    ...
    Sx = scattering.forward(
        x.squeeze().to(torch.float)
    )  # J=8 => [256, 337, 3], J=9 => [256, 435, 1]
    Sx = Sx[:, 1:, :]
    log_eps = 1e-6
...
def fft_torch(Xb):
    ...
    powers = (powers - powers.mean(axis=0, keepdim=True)) / powers.std(
        axis=0, keepdim=True
    )
    return powers
...
def pdf(x, mu, sigma):  # probability density function
    var = sigma ** 2
    return 1.0 / ((2 * math.pi * var) ** 0.5) * torch.exp(-((x - mu) ** 2) / (2 * var))
...
def findlocals(search, startframe=None, trace=False):
    ...
    frame = [
        tb for (tb, _, lineno, fname, _, _) in frames if search in (lineno, fname)
    ][0]
...
class update_mean_var_std_arr:
    ...
    def update(self, samp_new):
        ...
        mean_new = (
            1.0
            * (mean_samp * n_new + self.mean_prev * self.n_prev)
            / (n_new + self.n_prev)
        )
        ...
        var_new = (
            1.0
            * (
                self.n_prev * self.var_prev
                + self.n_prev * ((mean_new - self.mean_prev) ** 2)
                + n_new * var_samp
                + n_new * ((mean_new - mean_samp) ** 2)
            )
            / (self.n_prev + n_new)
        )
        std = var_new ** (0.5)
...
def load_lfp_rip_sec_samprate(lpath_lfp):
    ...
    lfp = (
        np.load(lpath_lfp).squeeze().astype(np.float32)
    )  # 2kHz -> int16, 1kHz, 500Hz -> float32
...
def load_lfps_and_rips_pad(
    fpaths_2kHz, samp_rate=1000
):  # input: npy, pkl (pands.DataFrame) -> output: torch.Tensor
    lfp = []
    rip_sec = []
    ...
    for i in range(len(fpaths_2kHz)):
        ...
        rip_sec_tmp = torch.tensor(
            np.array(rip_sec_tmp), dtype=torch.float32
        )  # float16 makes inf
        lfp.append(lfp_tmp)
        rip_sec.append(rip_sec_tmp)
    lfp = torch.nn.utils.rnn.pad_sequence(
        lfp, batch_first=True, padding_value=-float("Inf")
    )
    rip_sec = torch.nn.utils.rnn.pad_sequence(
        rip_sec, batch_first=True, padding_value=-float("Inf")
    )
...
def plt_ripple(
    ...
    for ripple in ripple_sec_plt.itertuples():
        try:
            ...
            ax.axvspan(
                ripple.start_sec, ripple.end_sec, alpha=0.3, color="red", zorder=1000
            )
...
def check_ripple(
    lfp, rip_sec, samp_rate, start_sec=0, end_sec=100, lowcut=100, highcut=250
):
    ...
    plt_ripple(
        ...
        start_sec=start_sec,
        end_sec=end_sec,
        title="{}-{} Hz bandpassed".format(lowcut, highcut),
        xlabel="Time [s]",
    )
...
def mk_ripple_dataloader(
    ...
    if collate_fn_class:
        ...
        dataloader = utils.DataLoader(
            ...
            pin_memory=True,
            collate_fn=collate_fn,
        )  # partial(collate_fn_tra, max_seq_len=max_seq_len
    else:
        dataloader = utils.DataLoader(
            dataset, batch_size=bs, shuffle=train, num_workers=nw, pin_memory=True
        )
...
def create_inputs_and_targets(lfp, rip_sec, max_seq_len):
    ...
    for i in range(len(lfp)):
        reshaped_lfp, dur, lat = reshape_an_lfp_and_get_targets(
            lfp[i], rip_sec[i], max_seq_len
        )
...
def mold_lfps_and_rips_sec(lfp, rip_sec, samp_rate, max_seq_len):
    ...
    shortest_length_aligned_by_last_rip_starts = (
        lengths_aligned_by_last_rip_starts.min()
    )
    first_cut_lengths = (
        lengths_aligned_by_last_rip_starts
        - shortest_length_aligned_by_last_rip_starts
        + max_seq_len
    )  #
    ...
    for i in range(len(lfp)):
        lfp[i] = lfp[i][
            first_cut_lengths[i] + reminder : lengths_aligned_by_last_rip_starts[i]
        ]
...
def reshape_an_lfp_and_get_targets(
    an_lfp, a_rip_sec, max_seq_len
):  # you can use this for train data with pool, map multiprocessing
    reshaped_lfp = []
    target_dur = []
    ...
    for i in range(int(an_lfp_length / max_seq_len)):
        try:
            ...
            ]  # fixme, reshape or view will be faster
            ...
            from_slice_end_to_the_next_ripple_start = (
                a_rip_sec[0][next_ripple_ind] - slice_end_sec
            )
            reshaped_lfp.append(sliced)
            target_dur.append(next_ripple_dur)
            target_from_slice_end_to_the_next_ripple_start.append(
                from_slice_end_to_the_next_ripple_start
            )
        except:
            pass
    ...
    target_from_slice_end_to_the_next_ripple_start = torch.stack(
        target_from_slice_end_to_the_next_ripple_start, dim=0
    )
    return (
        reshaped_lfp,
        target_dur,
...
def reshape_an_lfp_and_get_targets_wrapper(
    args,
):  # you can use this for train data with pool, map multiprocessing
...
def get_targets_from_reshaped_lfp(
    reshaped_lfp, rip_sec, molded_lfp_len, samp_rate, max_seq_len
):  # 50 min with 1 thread CPU
    target_dur = []
    target_lat = []
    for _i in range(len(reshaped_lfp)):
        ...
        next_ripple_idx = bisect_left(
            rip_sec[rip_sec_ind]["start_time"].values, slice_end_sec
        )
        next_ripple_dur = rip_sec[rip_sec_ind].iloc[next_ripple_idx]["duration"]
        from_slice_end_to_the_next_ripple_start = (
            rip_sec[rip_sec_ind].iloc[next_ripple_idx]["start_time"] - slice_end_sec
        )
...
def mk_dataset_from_an_lfp_and_a_rip(
    an_lfp, _a_rip_sec, samp_rate, max_seq_len, perturbation
):
...
def mold_to_dataset(lfps, rips_sec, samp_rate, max_seq_len, include_perturbation):
    ...
    arg_list = [
        (lfps[i], rips_sec[i], samp_rate, max_seq_len, perturbation[i])
...
def mk_dataloader(
    fpaths, samp_rate, max_seq_len, use_pertubation, bs, use_fp16, train
):  # fixed for each of Train dl or Test dl
    # lfps: list, len(lfps) n_load ->
    # molded_lfps [n_load, molded_lfp_len] ->
    # reshaped_lfps [n_load*(molded_lfp_len/max_seq_len), max_seq_len]
    lfps, rips_sec, samp_rate = load_lfps_and_rips(
        fpaths, samp_rate=samp_rate
    )  # There is another version using padding.
    Xb, Tb_dur, Tb_lat = mold_to_dataset(
        lfps, rips_sec, samp_rate, max_seq_len, use_pertubation
    )
    del lfps, rips_sec
    gc.collect()
    if use_fp16:
        Xb, Tb_dur, Tb_lat = (
            ...
            Tb_dur.astype(np.float16),
            Tb_lat.astype(np.float16),
        )
    dataloader = pack_to_dataloader(
        Xb,
        Tb_dur,
        Tb_lat,
        train=train,
...
def plot_learning_curve(d, spath=None):
    # axis
    x_tra = [
        d["bs_tra"] * d["log_interval"] * i for i in range(len(d["losses_dur_tra"]))
    ]
    ...
    ax[0].legend(
        ["Train Loss isnextRipple", "Test Loss isnextRipple"], loc="upper right"
    )
    # duration
...
def load_an_lfp_and_a_rip(
    fpath_2kHz, samp_rate=1000
):  # input: npy, pkl (pands.DataFrame) -> output: torch.Tensor
...
def test():
    ...
    Xb, Tb_dur, Tb_lat = mk_dataset_from_an_lfp_and_a_rip(
        lfp, rip_sec, 1000, 1000, True
    )
...
def calc_roc_curve(y_true, y_score, spath=None, plot=False):
    ...
    if plot:
        ...
        lw = 2
        plt.plot(
            fpr, tpr, lw=1, label="ROC curve (area = %0.2f)" % roc_auc, marker="o"
        )  # color='darkorange',
...
def calc_precision_recall_curve(y_true, probas_pred, pos_label=None, plot=False):
    ...
    if plot:
        plt.plot(
            recall, precision, label="PRE-REC curve (area = %0.2f)" % pr_auc, marker="o"
        )  # color='darkorange',)
        plt.xlabel("Recall")
...
def generate_random_str(n_str=8):
    ...
    random_str = "".join(
        [random.choice(string.ascii_letters + string.digits) for n in range(n_str)]
    )
    return random_str
