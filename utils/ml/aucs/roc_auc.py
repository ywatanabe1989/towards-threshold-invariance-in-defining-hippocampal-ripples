...
def calc_roc_auc(true_class, pred_proba, labels, plot=False):
    ...
    for i in range(n_classes):
        fpr[i], tpr[i], threshold[i] = roc_curve(true_class[:, i],
                                                 pred_proba[:, i])
    ...
    fpr["micro"], tpr["micro"], threshold["micro"] = \
        roc_curve(true_class.ravel(), pred_proba.ravel())
    roc_auc["micro"] = \
        roc_auc_score(true_class, pred_proba, average="micro")
    print('Average fpr score, micro-averaged over all classes: {0:0.2f}'
          .format(roc_auc["micro"]))

    # macro
    roc_auc["macro"] = \
        roc_auc_score(true_class, pred_proba, average="macro")
    print('Average fpr score, macro-averaged over all classes: {0:0.2f}'
    ...
    if plot:
        ...
        l, = ax.plot(np.linspace(0.01, 1), np.linspace(0.01, 1), color='gray', lw=2,
                      linestyle='--', alpha=.8)
        lines.append(l)        
