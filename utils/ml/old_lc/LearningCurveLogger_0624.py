...
class LearningCurveLogger(object):
    ...
    def plot_learning_curves(
        ...
        ## Epoch hline # koko
        indi_global_for_epoch_start = (
            np.where(self.dfs_pivot["Training"]["i_epoch"].diff(-1) == -1.0)[0] + 1
        )
        indi_global_for_epoch_start = np.hstack([0, indi_global_for_epoch_start])

        for i_ax in range(len(ax)):
            vmin, vmax = ax[i_ax].get_ylim()
            print(vmin, vmax)
            ax[i_ax].vlines(
                indi_global_for_epoch_start,
                vmin,
            ...
            # ax[i_ax].axvline(indi_global_for_epoch_start, color="black", label='epoch start')
...
if __name__ == "__main__":
    ...
    # indi_global_for_epoch_start = np.hstack([
    #     np.array([0]),
    #     np.array([1000, 2000]),
    # ])
